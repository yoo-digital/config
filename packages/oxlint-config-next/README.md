...
[![npm version](https://badge.fury.io/js/%40yoo-digital%2Foxlint-config-next.svg)](https://www.npmjs.com/package/@yoo-digital/oxlint-config-next)

Contains the settings and rules used for Next.js projects within YOO.
It is publicly available via the npm registry.

## Purpose

The exported `oxlint.config.ts` file contains an Oxlint config that is based on the nextjs plugins.
...
```text
npx install-peerdeps --dev @yoo-digital/oxlint-config-next
```
...
```javascript
import nextConfig from '@yoo-digital/oxlint-config-next';
...
**Important**: This configuration should only share the Oxlint configuration for Next.js projects.
