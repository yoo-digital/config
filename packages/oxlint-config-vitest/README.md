...
[![npm version](https://badge.fury.io/js/%40yoo-digital%2Foxlint-config-vitest.svg)](https://www.npmjs.com/package/@yoo-digital/oxlint-config-vitest)

Contains the settings and rules used for projects within YOO that use vitest for testing.
It is publicly available via the npm registry.

## Purpose

The exported `oxlint.config.ts` file contains an Oxlint config that is based on the vitest plugin.
...
```text
npx install-peerdeps --dev @yoo-digital/oxlint-config-vitest
```
...
```javascript
import vitestConfig from '@yoo-digital/oxlint-config-vitest';
...
**Important**: This configuration should only share the Oxlint configuration for projects using vitest for testing.
